...
import { RULE_FACTORIAL_V2 as P, buildRuleFactorialV2Schedule, decodeCondition,
  directionalEndpoints } from "./rule_given_factorial_v2_protocol.js";
  ...
  if (factors.calculator_available) {
    ...
    if (!calculation.model_stage_error) {
      ...
      try {
        ...
        messages.push({ role: "assistant", content: calculation.message.content ?? null,
...
async function liveDryRun() {
  ...
  const result = { schema: "ik.rule-given-factorial-v2-live-dry-run.v1", completed_at: new Date().toISOString(),
...
async function promptIntegrityGate() {
  ...
  for (const item of schedule) {
    ...
    for (const condition of P.conditions) {
      ...
      if (factors.calculator_available) messages.push({ role: "assistant", content: null,
  ...
  const result = { schema: "ik.rule-given-factorial-v2-integrity-gate.v1", passed: true,
...
function assemble() {
  ...
  const artifact = { schema: "ik.rule-given-factorial-v2-batch.v1", run_id: P.run_id,
...
if (!dry.passed) {
  fs.writeFileSync(path.join(outputDir, "abort.json"), `${JSON.stringify({ schema: "ik.rule-given-factorial-v2-abort.v1",
  ...
  throw new Error("V2 live dry-run failed; abort before experimental prediction 1");
}
...
for (const item of schedule) {
  ...
  for (const condition of item.condition_order) {
    ...
    const sealed = { schema: "ik.rule-given-factorial-v2-prediction.v1",
