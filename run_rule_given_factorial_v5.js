...
import { RULE_FACTORIAL_V5 as P, buildRuleFactorialV5Schedule, decodeCondition,
  directionalEndpoints } from "./rule_given_factorial_v5_protocol.js";
...
const calculatorTool = { type: "function", function: { name: "calculate_vector",
  description: "Pure vector arithmetic. Choose one operation. negate returns -values; multiply_scalar returns values*scalar; add_scalar returns values+scalar; classify_threshold labels supplied values rise/fall/stable around Ãƒâ€šÃ‚Â±threshold.",
...
const recorderResponseFormat = { type: "json_schema", json_schema: {
  name: "directions_only", strict: true, schema: recordTool.function.parameters
} };
...
function requestBody({ messages, tools, toolChoice, thinking, maxTokens, responseFormat = null }) {
...
async function predict(evidence, condition, ledger, label) {
  ...
  const recording = await complete({ messages, tools: null, toolChoice: null,
    thinking: false, maxTokens: P.recorder_max_tokens, responseFormat: recorderResponseFormat },
    `${label}_${condition}_recorder`, ledger);
  ...
  if (recording.model_stage_error) invalid_output = `HTTP ${recording.http_status}: ${recording.body}`;
    ...
    try {
      parsed = parseDirections({ function: { name: recordTool.function.name,
        arguments: recording.message.content } });
...
async function liveDryRun() {
  ...
  const result = { schema: "ik.rule-given-factorial-v5-live-dry-run.v1", completed_at: new Date().toISOString(),
...
async function promptIntegrityGate() {
  ...
  for (const item of schedule) {
    ...
    for (const condition of P.conditions) {
      ...
      const tokens = (await renderPromptTokenMap(baseUrl, { messages,
  ...
  const result = { schema: "ik.rule-given-factorial-v5-integrity-gate.v1", passed: true,
...
function assemble() {
  ...
  const artifact = { schema: "ik.rule-given-factorial-v5-batch.v1", run_id: P.run_id,
...
if (!dry.passed) {
  fs.writeFileSync(path.join(outputDir, "abort.json"), `${JSON.stringify({ schema: "ik.rule-given-factorial-v5-abort.v1",
  ...
  throw new Error("V5 live dry-run failed; abort before experimental prediction 1");
}
...
for (const item of schedule) {
  ...
  for (const condition of item.condition_order) {
    ...
    const sealed = { schema: "ik.rule-given-factorial-v5-prediction.v1",
