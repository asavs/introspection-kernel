...
import { RULE_FACTORIAL_V4 as P, buildRuleFactorialV4Schedule }
  from "./rule_given_factorial_v4_protocol.js";
...
const preregistration = {
  ...
  thesis_question: "With calculator-before-thinking ordering, exact valid message replay, verbatim completed-thought transfer, and a directions-only recorder, can Qwen3-8B execute the supplied scale-zero JVP rule?",
  predecessor_abort: {
    ...
    observed: [
      "all four calculator cells made a valid required calculator call",
      "all four calculators selected classify_threshold and returned the correct synthetic direction vector",
      "the V3 controller inserted reasoning_content:null although the model response omitted that property",
      "llama.cpp rejected every downstream replay containing that invented null field"
    ]
  },
  ...
  pre_prediction_gates: {
    ...
    failure_action: "abort V4 before prediction 1; no post-gate patching"
  },
  endpoints: {
  ...
  randomization: { schedule: buildRuleFactorialV4Schedule() },
...
if (process.argv.includes("--verify")) {
  ...
  if (JSON.stringify(existing.randomization.schedule) !== JSON.stringify(buildRuleFactorialV4Schedule())) throw new Error("schedule drift");
